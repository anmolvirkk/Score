...
export default function TopPicks ({heading} : {heading:string}) {
  const svgs = [
  ...
  const Rank = ({index, name, bet, stats} : {index:number, name:string, bet:string, stats: {type:string, goals:string}[]}) => {
    return (
      <div className="flex items-end">
        <div className="w-[10rem] h-[10rem]">{svgs[index]}</div>
        <div className="text-white bg-primary-700 p-[1rem] flex flex-col gap-[1rem]">
          <div className="flex flex-wrap gap-[0.5rem]">
            {stats.map((item, index) => (
              <div key={index} className="flex gap-[1rem] items-center justify-between w-full bg-primary-600 p-[1rem]">
                <div className="opacity-[0.5] leading-[1] text-[0.8rem]">{item.type}</div>
                <div className="font-[900] leading-[1] text-[0.8rem]">{item.goals}</div>
              </div>
            ))}
          </div>
          <div className="flex flex-col gap-[0.5rem]">
            <div className="text-[1.5rem] font-[900] leading-[1]">{bet}</div>
            <div className="leading-[1.5] text-[0.8rem] opacity-[0.3]">{name}</div>
          </div>
        </div>
      </div>
    )
  ...
  return (
    <>
      ...
      <Swiper
        ...
        slidesPerView={Math.max(window.innerWidth/(19.65*16))}
      >
        {ranks.map((item, index) => (
          <SwiperSlide key={index}><Rank {...item} index={index} /></SwiperSlide>
        ))}
      </Swiper>
    </>
  )
